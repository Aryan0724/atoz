...
async function run() {
  ...
  console.log('Navigating to wedding card customizer page...');
  ...
  // Target 'Groom's Name' or any name
  const nameElementSelector = '#template-preview-container > div > div';
  
  // Find a suitable element to drag
  let targetIdx = elements.findIndex(e => e.text && (e.text.toLowerCase().includes('groom') || e.text.toLowerCase().includes('bride') || e.text.toLowerCase().includes('wedding') || e.text.toLowerCase().includes('save the date')));
  ...
  if (finalStyle.left === elements[targetIdx].left && finalStyle.top === elements[targetIdx].top) {
    ...
    console.log('SUCCESS: Wedding Card element moved successfully!');
  }
