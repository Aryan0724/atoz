...
async function run() {
  ...
  console.log('Navigating to wedding card customizer page with new landscape template 30 (Elite Floral 01)...');
  await page.goto('http://localhost:3000/customize/wedding-card-custom?template=30', {
  ...
  console.log('Found elements on Wedding Card canvas:', JSON.stringify(elements, null, 2));

  // Check the aspect ratio of the preview card container
  const containerSize = await page.evaluate(() => {
    const container = document.querySelector('#template-preview-container')?.parentElement;
  ...
  console.log('Container preview size details:', containerSize);

  // Target 'groom' (which is 'Prashant Sharma' or something similar) or the first name element
  const nameElementSelector = '#template-preview-container > div > div';
  
  let targetIdx = elements.findIndex(e => e.text && e.text.includes('Sharma'));
  if (targetIdx === -1 && elements.length > 0) {
    targetIdx = elements.findIndex(e => e.text && e.text.length > 2);
  }

  if (targetIdx === -1) {
    console.error('Could not find any text elements to drag on Wedding Card canvas!');
    await browser.close();
    return;
  }
  ...
  // Click down
  console.log(`Dragging from (${startX}, ${startY}) to (${startX + 100}, ${startY + 50})`);
  ...
  if (finalStyle.left === elements[targetIdx].left && finalStyle.top === elements[targetIdx].top) {
    ...
    console.log('SUCCESS: Landscape Wedding Card element moved successfully!');
  }
