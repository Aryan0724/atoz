...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        // 2. Create profile in our custom profiles table (if not handled by trigger)
        ...
        // Note: In a production Supabase setup, we'd use a database trigger to create the profile.
        // We'll catch error but proceed since profile might exist via trigger.
    ...
    <div className="min-h-screen bg-brand-lightGray flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-gray-500">
          Join India&apos;s leading print on demand platform.
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white py-12 px-10 shadow-2xl rounded-[40px] border border-gray-100">
          ...
          <form className="space-y-6" onSubmit={handleRegister}>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide">Full Name / Brand Name</label>
              <input 
                type="text" 
                required
                value={fullName}
            ...
            <div className="pt-2">
              ...
              >
                {loading ? <Loader2 className="h-6 w-6 animate-spin" /> : 'Create Free Account'}
              </button>
            </div>
          </form>
          ...
          <div className="mt-10 text-center">
            <p className="text-sm font-bold text-gray-500">
              Already have an account? <Link href="/login" className="text-brand-pink hover:underline">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
