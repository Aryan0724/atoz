    ...
    <div className={cn(
      ...
      <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-4 z-50 flex-shrink-0">
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="h-6 w-px bg-gray-100 mx-0.5 md:mx-1" />
          <button onClick={() => canvasRef.current?.undo?.()} className="p-2 text-gray-400 hover:text-brand-dark transition-colors"><RotateCcw className="h-4 w-4 md:h-5 md:w-5 -scale-x-100" /></button>
          <button onClick={() => canvasRef.current?.redo?.()} className="p-2 text-gray-400 hover:text-brand-dark transition-colors"><RotateCcw className="h-4 w-4 md:h-5 md:w-5" /></button>

          <div className="md:hidden h-6 w-px bg-gray-100 mx-1" />
          <button 
            onClick={() => setIsUiHidden(!isUiHidden)} 
            className={cn(
              "md:hidden p-2 rounded-lg transition-colors",
              isUiHidden ? "text-brand-pink bg-pink-50" : "text-gray-400"
            )}
          >
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-2 px-4 py-1.5 bg-gray-50 hover:bg-brand-pink hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all italic"
          >
      ...
      <div className={cn(
        ...
        <main className={cn(
          ...
          {!isTemplateForm && (
            <div className="absolute top-0 left-0 right-0 z-20 flex flex-wrap justify-center p-2 md:p-4 pointer-events-none">
              <div className={cn(
                ...
                activeObject ? "translate-y-0 opacity-100 scale-100" : "-translate-y-4 opacity-0 scale-95 pointer-events-none"
              )}>
                <TopToolbar 
                  activeObject={activeObject}
          ...
          <div className={cn(
            ...
            {isOutOfBounds && (
              <div className="absolute top-20 md:top-24 left-1/2 -translate-x-1/2 z-40 bg-orange-500 text-white px-4 py-2 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-xl animate-bounce whitespace-nowrap">
                Outside Print Area
              </div>
            )}
                  ...
                  {[
                    ...
                    { id: 'right', label: 'Right' },
                      ...
                      <button
                        ...
                        className={cn(
                          "px-4 py-1.5 md:px-5 md:py-2 rounded-full text-[10px] md:text-[11px] font-black tracking-tight leading-none transition-all flex-shrink-0",
                          activeView === v.id 
            ...
            <div className={cn(
              ...
              (product as any).design_mode === 'template_form' 
                ? "fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-2xl border-t border-gray-100 p-4 md:p-6 flex items-center justify-center shadow-[0_-20px_40px_rgba(0,0,0,0.03)]" 
                : "absolute bottom-4 right-4 md:bottom-8 md:right-8 scale-[0.85] md:scale-100 origin-bottom-right",
              isUiHidden && "translate-y-[150%] opacity-0 pointer-events-none"
            )}>
               <div className={cn(
                 ...
                 <div className={cn(
                   "bg-white/95 backdrop-blur-2xl px-4 py-3 md:px-6 md:py-4 rounded-[24px] md:rounded-[32px] border border-white/40 shadow-2xl shadow-brand-dark/5 ring-1 ring-black/5 flex items-center gap-4 md:gap-8",
                   (product as any).design_mode === 'template_form' ? "w-auto" : "w-64 md:w-80 flex-col gap-2 md:gap-4"
                 )}>
                    {/* Quantity */}
                    <div className="flex items-center gap-3 md:gap-4">
                       <span className="text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Qty</span>
                       <div className="flex items-center gap-2 md:gap-3 bg-[#f7f7f2] rounded-full p-1 border border-gray-100">
                          <button 
                            onClick={() => setQuantity(q => Math.max((product.moq || 1), q - 1))}
                            className="w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center bg-white text-gray-500 hover:text-brand-dark hover:shadow-sm transition-all"
                          >
                           ...
                           <input 
                             ...
                             className="w-8 md:w-12 bg-transparent text-[10px] md:text-xs font-black text-brand-dark text-center outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                           />
                          <button 
                    ...
                    <div className="flex flex-col">
                       <span className="text-[8px] md:text-[9px] font-black text-brand-pink/50 uppercase tracking-[0.2em] md:tracking-[0.25em] mb-0.5">
                         {selectedQuality} • {discountPercent > 0 ? `${discountPercent}% OFF` : 'Base'}
                       </span>
                       <div className="flex items-baseline gap-1 md:gap-2">
                          <span className="text-[10px] md:text-sm font-black text-brand-dark italic">₹</span>
                          <span className="text-xl md:text-3xl font-black text-brand-dark tracking-tighter italic leading-none">{totalPrice.toLocaleString()}</span>
                          <span className="hidden sm:inline text-[8px] md:text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-1 md:ml-2">₹{unitPrice}/unit</span>
                       </div>
                    </div>
                 </div>
                 ...
                 <button 
                   ...
                   className={cn(
                     "px-6 py-4 md:px-10 md:py-5 bg-brand-dark text-white rounded-[20px] md:rounded-[24px] text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] md:tracking-[0.25em] shadow-xl md:shadow-[0_20px_40px_rgba(0,0,0,0.15)] hover:bg-brand-pink transition-all flex items-center gap-2 md:gap-3",
                     isFinishing && "opacity-50"
                   )}
                 >
                    {isFinishing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                    {isFinishing ? "Processing..." : "Finish Project"}
                 </button>
               </div>
            </div>
          </div>
      ...
      <div className={cn(
        "md:hidden flex flex-col flex-shrink-0 bg-white border-t border-gray-100 z-50 pb-safe relative transition-transform duration-500",
        isUiHidden && "translate-y-full"
      )}>
          {mobilePanel && (
            <div className="absolute inset-x-0 bottom-full bg-white z-[60] h-[55vh] shadow-[0_-12px_40px_rgba(0,0,0,0.12)] rounded-t-[32px] overflow-hidden flex flex-col animate-in slide-in-from-bottom-full duration-500 cubic-bezier(0.4, 0, 0.2, 1)">
          ...
          <div className="flex items-center justify-around px-2 py-1 bg-white/90 backdrop-blur-xl relative z-50 border-t border-gray-100">
            {mobileTools.map(tool => (
              <button 
                key={tool.id}
                onClick={() => setMobilePanel(mobilePanel === tool.id ? null : tool.id)}
                className={cn(
                  "flex flex-col items-center gap-1 p-2 min-w-[64px] rounded-2xl transition-all",
                  mobilePanel === tool.id ? "text-brand-pink bg-pink-50 shadow-sm scale-105" : "text-gray-400 active:scale-95"
                )}
              >
                <div className={cn(
                  "transition-transform",
                  mobilePanel === tool.id ? "scale-110" : "scale-100"
                )}>
                  {tool.icon}
                </div>
      ...
      {isSuccess && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[40px] shadow-2xl flex flex-col items-center gap-3 md:gap-4 animate-in zoom-in duration-300 max-w-[90%]">
            <div className="w-12 h-12 md:w-16 md:h-16 bg-[#f0f0e8] rounded-full flex items-center justify-center">
              <Check className="h-6 w-6 md:h-8 md:w-8 text-brand-olive" />
            </div>
            <h2 className="text-xl md:text-2xl font-black text-brand-dark tracking-tighter">Design Saved</h2>
            <p className="text-gray-500 text-xs md:text-sm text-center">Your product is ready in the cart.</p>
          </div>
        </div>
      )}
    </div>
